	...
	<ul>
		...
		<li><a href="#accessing-trees-and-tuples">Accessing Trees (and Tuples)</a>
			<ul>
				...
				<li><a href="#conservatively-an-explicit-variable-for-every-branch">Conservatively: An <strong>Explicit Variable</strong> for Every Branch</a></li>
				...
				<li><a href="#easier-roots-built-in-functionality">Easier: ROOT’s <strong>Built In</strong> Functionality</a></li>
				<li><a href="#the-classic-roots-double-chevron-draw">The Classic: ROOT’s Double Chevron <strong>Draw</strong></a></li>
...
A way to handle big chunks of physics data in ROOT is [NTuples](http://root.cern.ch/root/html/TNtuple.html). An NTuple (or Tuple, or **Tree**) usually contains one quantity (e.g. the mass of a particle), ordered into different events. Trees are *the* data structure for accessing data with ROOT and are hence built very well into it. Once your data is in trees, ROOT offers very efficient means of accessing, manipulating and displaying them.

But there are different ways to do so, with different pros and cons, and different performances. This post gives a short overview about the techniques I know of.
...
{% endhighlight %}
After initiating a file like this, objects inheriting from `TObject` (e.g. `TH1`s) are saved to the file when calling `file->Write()`. See the [`basic2.C` tutorial](http://root.cern.ch/root/html/tutorials/tree/basic2.C.html) for an easy example.
...
Usually, before creating good-looking histograms for the last point, I have a look at the quantity in an interactive `TBrowser` windows the of `reco.root` data. My working, hence, is a mix of using ROOT's quick but unreliable interactive abilities and programmable and reproducible macro mode.
...
The way, most people are using ROOT's tree. The way, ROOT's `tree->MakeClass("myclass")` also creates the data structure inside the `myclass.h` and `myclass.C`.
...
{% endhighlight %}
Look! No loop! No `float` variables! Just the histogram, and projection onto that. Unfortunately, this uses ROOT's ugly string object identifier method, but that's what we all have to live with all the time anyways. You can even plot under conditions (by adding a third parameter inside the parentheses, like `, "dPlusPdgCode > 0"`). To plot two-dimensionally, use a colon ("dPlusMass:dPlusMassSmeared") – of course our histogram has to be a `TH2F` as well.  
If that sounds awfully familiar to you, well, see the next part.

 * **Pro**: Very short; no intermediate variable definition
 * **Con**: Uses ROOT's string syntax; only limited ability to manipulate data for histogram (like `(dPlusMass - dPlusMassSmeared)**2+4)`.

### The Classic: ROOT's Double Chevron `Draw`
...
{% endhighlight %}
Notice the undocumented `goff` drawing option, which draws *silently* and does not open a new canvas while drawing – pretty handy.

 * **Pro**: Very short; no intermediate variable definition
 * **Con**: Uses even more so ROOT's string syntax; data manipulation limits as above
...
ROOT also offers built-in functionality to directly transfer a branch's content onto a histogram. Use `tree->Draw()` or the wrapper function `tree->Project()` for this.
